...
export function MarcarCompromisso1Pedido() {
  const location = useLocation();
  const navigate = useNavigate();
  const quantidadesIniciais = location.state.pedido.itemList.map(() => 0);
  ...
  const somaQuantidade = (index) => {
    setQuantidades((prevQuantidades) => {
      ...
      if (
        newQuantidades[index] < location.state.pedido.itemList[index].quantidade
      ) {
  ...
  const submit = (e) => {
    ...
    return navigate("/marcar-compromisso-pedido-2", {
      state: { itemPropostaRequestDtoList, pedido },
    });
  };
              ...
              {isActive && (
                <>
                  ...
                  <div className="bg-white absolute top-11 -right-1 flex flex-col mb-2 border-[1.5px] rounded-lg px-3 py-2">
                    <Link
                      to={`/denunciar-pedido/${location.state.pedido.id}`}
                      state={{ tipoAnuncio: 2 }}
                      className="flex"
                    >
