...
export function MarcarCompromisso1Doacao() {
  ...
  const somaQuantidade = (index) => {
    setQuantidades((prevQuantidades) => {
      ...
      if (
        newQuantidades[index] < location.state.doacao.itemList[index].quantidade
      ) {
  ...
  const submit = (e) => {
    ...
    return navigate("/marcar-compromisso-doacao-2", {
      state: { itemPropostaRequestDtoList, doacao },
    });
  };
